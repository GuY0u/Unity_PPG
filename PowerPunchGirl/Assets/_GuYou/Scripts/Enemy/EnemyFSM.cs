...
public class EnemyFSM : MonoBehaviour
{
    ...
    private void Return()
    {
        ...
        //시작위치까지 도달하지 않을때는 이동
        //도착하면 대기상태로 변경
        if (Vector3.Distance(transform.position, startPoint) > 1.7f)
        {
    ...
    public void HitDamage(int value)
    {
        ...
        //hudText.GetComponent<FloatingDamage>().damage = value; // 데미지 전달
        if (FloatingTextPrefab && hp > 0)
        {
            Vector3 dis = lookCam.position - this.transform.position;
            dis.Normalize();
            var go = Instantiate(FloatingTextPrefab, transform.position, Quaternion.LookRotation(dis), transform);
