...
public class Stereo3D : MonoBehaviour {
	...
	public void SetDefault () {
		_leftCam.GetComponent<Camera>().CopyFrom(_cameraBody.NodalCamera);
		_rightCam.GetComponent<Camera>().CopyFrom(_cameraBody.NodalCamera);
		Camera camera = _cameraBody.NodalCamera;
		
		//_leftCam.camera.cullingMask = (LayerMask)(7);
		//_rightCam.camera.cullingMask = (LayerMask)(7);
		
		_leftCam.GetComponent<Camera>().clearFlags = CameraClearFlags.Skybox;
		_rightCam.GetComponent<Camera>().clearFlags = CameraClearFlags.Skybox;
