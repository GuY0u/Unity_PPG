...
public class PushTrigger : MonoBehaviour {
    ...
    void OnTriggerEnter(Collider trig)
    {
        ...
        Debug.Log(this.name + " has been hit by " + trig.name);
        // check if Key pressed and collider hit was from correct target
        if (trig == _activator)
        {
            // set the door ready to move
            Debug.Log(this.name + " door has been opened");
            _iscolliding = true;
        }
        else
            Debug.Log(this.name + " door has not been opened because " + trig.name + " is not the same as activator: " + _activator.name);
    }
